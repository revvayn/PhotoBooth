...
file_put_contents('D:/PROJEK/PhotoBooth/storage/app/realframe.html', $bRun?);
preg_match('/data(?:-category|category)="/', $b, $m);
echo "kategori tombol: " . ($m ? 'ADA' : 'TIDAK ADA') . "\n";
preg_match_all('/data-frame-option="([^"]+)"/', $b, $m);
echo "frame-options: " . implode(',', $m[1] ?? []) . "\n";